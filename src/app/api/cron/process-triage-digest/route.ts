...
export async function GET(request: NextRequest) {
  ...
  for (const [userId, notifications] of grouped) {
    try {
      ...
      const classifiedMessages: ClassifiedMessage[] = notifications.map(
        (n) => {
      ...
      await updateWorkflowRun(workflowRun.id, {
        ...
        metricsJson: {
          ...
          readOnly: classifiedMessages.filter(
            (m) => m.category === "READ_ONLY"
          ).length,
          noise: classifiedMessages.filter((m) => m.category === "NOISE")
            .length,
        },
      });
